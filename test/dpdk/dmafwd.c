...
// 描述一个端口的信息，包括端口号，队列数，用于软件复制模式的rtering指针
// 以及用于硬件复制模式的DMAdev is数组
...
// 启用的端口掩码
// 掩码是表示有哪些端口被启用了
// 比如: 0x03(只有0和1端口被用了) 0x01(只有0端口被启用了)
...
// 存储了全局的DMA缓冲区信息，包括发送和复制的m_buf数组、以及已发送的计数
struct dma_bufs {
...
// 
// 更新mbuf的mac地址确保他能够发往正确的地方
// 更新数据包中的以太网头部的源地址和目的地址
static void
update_mac_addrs(struct rte_mbuf *m, uint32_t dest_portid)
{
	 ...
	 */
	// 02:00:00:00:00:xx 代表的是某个dma是被的
	tmp = &eth->dst_addr.addr_bytes[0];
	*((uint64_t *)tmp) = 0x000000000002 + ((uint64_t)dest_portid << 40/* 5*8 */);
...
{
	int32_t ret;
	uint32_t nb_rx/*接收到数据包的数量*/, nb_enq/*成功入队的数据包的数量*/, i, j;
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];	// 存储从网卡接收的数据包
	struct rte_mbuf *pkts_burst_copy[MAX_PKT_BURST];	// 存储复制后的数据包
	...
	for (i = 0; i < rx_config->nb_queues; i++) {
		...
		// 如果没有从网卡接收任何数据包，则尝试从DMA设备中获取已复制的数据包
		// 如果成功获取到数据包，则跳转到handle_tx标签继续处理
		if (nb_rx == 0) {
			if (copy_mode == COPY_MODE_DMA_NUM &&
		...
		// 将原始的数据包的元数据复制到新分配的数据包
		for (j = 0; j < nb_rx; j++)
			pktmbuf_metadata_copy(pkts_burst[j],
				pkts_burst_copy[j]);

		/** 根据copy_mode的不同，选择硬件复制或软件复制 */
		/** 如果使用硬件复制，则将数据包入队列以进行硬件复制，并更新统计信息 */
		/** 如果使用软件复制，则在内存中复制数据包 */
		if (copy_mode == COPY_MODE_DMA_NUM) {
